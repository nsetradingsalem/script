...
,'LT','LTI','LTTS','LUPIN','MARICO','MARUTI','MCDOWELL-N','MCX','MFSL','MGL','MINDTREE','MPHASIS','MRF','MUTHOOTFIN','NAUKRI'
...
def equity(request):
    ...
    callCrossed_odd = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=True,change_perc__gte = 3).filter(strike="Call Crossed").order_by('-time')
    putCrossed_odd = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=True,change_perc__lte = -3).filter(strike="Put Crossed").order_by('-time')
    callCrossed_even = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=False,change_perc__gte = 3).filter(strike="Call Crossed").order_by('-time')
    putCrossed_even = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=False,change_perc__lte = -3).filter(strike="Put Crossed").order_by('-time')
    ...
    calleven = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=False,change_perc__gte = 3).filter(strike="Call 1 percent").filter(change_perc__gte=2).order_by('section')  
    callodd = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=True,change_perc__gte = 3).filter(strike="Call 1 percent").filter(change_perc__gte=2).order_by('section') 
    puteven = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=False,change_perc__lte = -3).filter(strike="Put 1 percent").filter(change_perc__lte=-2).order_by('section')  
    putodd = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=True,change_perc__lte = -3).filter(strike="Put 1 percent").filter(change_perc__lte=-2).order_by('section') 
